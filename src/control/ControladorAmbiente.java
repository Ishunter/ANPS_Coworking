...
public class ControladorAmbiente {
    ...
    public void excluir(String nome) throws ExceptionExcluidoComSucesso, ExceptionCancelar, Exception {
        if (isNomeInUse(nome)) {
            boolean atualizarDados = CtrlUtil.getInstance().PanelYesOrNo(
                "Deseja excluir o Ambiente " + nome + "?",
                "Deseja excluir?");
            if (atualizarDados) {
                if (AmbienteDAO.getInstance().delete(AmbienteDAO.getInstance().read(nome))){
                    throw new ExceptionExcluidoComSucesso("Ambiente");
                } else {
                    throw new Exception("Não é possivel excluir um ambiente locado.");
                }
            } else {
                throw new ExceptionCancelar();
            }
