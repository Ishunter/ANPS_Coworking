...
public class LocacaoDAO {
    ...
    public boolean RlRecursosEstaLocado(String recurso) {
        Connection conn = control.ConexaoBD.getConnection();

        String sql = "SELECT * FROM public.rl_recursos WHERE recurso=? ";
        try (PreparedStatement queryRecursos = conn.prepareStatement(sql)) {
            queryRecursos.setString(1, recurso);
            ResultSet rs = queryRecursos.executeQuery();
            while (rs.next()) {
               return true;
            }
            ...
            System.err.println("locacaoDAO.RlRecursosGetAll() -> " + ex.getMessage());
            JOptionPane.showMessageDialog(null, "Problema ao ler relação de recurso do banco.");
        }
        return false;
    }
}
